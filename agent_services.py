class ComponentOutput:
    def __init__(self, display_name=None, value=None, highlight=None, score=0,
        ...
        # normalized_confidence = normalize_confidence(score, value)
        logger.info(f'value is {value} - score is {score}')
        self.score = int(round(score * 100))
        self.value = value
        ...
        self.name = name
        self.index = index
        self.sub_attr_output = {}
        self.page_header = page_header
    def set_sub_attr_output(self, sub_attr_name, sub_attr_out):
        try:
            if sub_attr_name in self.sub_attr_output:
                self.sub_attr_output[sub_attr_name].append(sub_attr_out.__dict__)
            else:
                self.sub_attr_output[sub_attr_name] = [sub_attr_out.__dict__]
...
class CAOutputBuilder:
    ...
    def _process_nested_group_section(self, sub_name, sub_meta, page, page_content, item):
        ...
        for sub_sub_name, sub_sub_meta in sub_meta.items():
            val, score, highlight = self._extract_val_score_highlight_from_sub_sub_meta(
                sub_sub_meta, page, page_content, item
            )
            if isinstance(val, list) and len(val) > 1:
                deep_section = self.set_attribute_output(sub_sub_name, value=None, group_header=True)
                for idx, single_val in enumerate(val, start=1):
                    indexed_name = f"{sub_sub_name} - {idx}"
                    ...
                    deep_section.set_sub_attr_output(indexed_name, sub_attr)
                nested_section.set_sub_attr_output(sub_sub_name, deep_section)
    ...
    def _process_group_dict_field(self, field_name, field_val, page, page_content, item, entities):
        ...
        for sub_name, sub_meta in field_val.items():
            if looks_like_group_dict(sub_meta):
                ...
                if nested_section.sub_attr_output:
                    section_output.set_sub_attr_output(
                        nested_section.name or nested_section.display_properties.get('display_name'),
                        nested_section
                    )
            else:
                ...
                if isinstance(val, list) and len(val) > 1:
                    ...
                    for idx, single_val in enumerate(val, start=1):
                        ...
                        # If single_val is a row dict (group of fields), create a group node
                        # and process each field inside it — don't store the dict as attr_value
                        if isinstance(single_val, dict) and looks_like_group_dict(single_val):
                            row_node = self.set_attribute_output(
                                attribute_name=indexed_name,
                                value=None,
                                group_header=True
                            )
                            for field_name, field_meta in single_val.items():
                                if looks_like_group_dict(field_meta):
                                    ...
                                    row_node.set_sub_attr_output(field_name, child)
                                else:
                                    ...
                                    row_node.set_sub_attr_output(field_name, child)
                            nested_section.set_sub_attr_output(indexed_name, row_node)
                        else:
                            # scalar value — original path is fine
                            sub_attr = self.set_attribute_output(
                                attribute_name=indexed_name,
                                value=single_val,
                                score=score,
        ...
        if section_output.sub_attr_output:
            entities.set_sub_attr_output(
                section_output.name or section_output.display_properties.get('display_name'),
                section_output
            )
    ...
    def _process_list_item_field(self, field_name, field_val, page, page_content, item, entities):
        # Case 1: list of child objects → indexed sub-attributes
        ...
        if isinstance(val, list) and len(val) > 1:
            section_output = self.set_attribute_output(field_name, value=None, group_header=True)
            for idx, single_val in enumerate(val, start=1):
                indexed_field_name = f"{field_name} - {idx}"
                attr_output = self.set_attribute_output(
                    attribute_name=indexed_field_name,
                    value=single_val,
                    score=score,
            ...
            entities.set_sub_attr_output(
                attr_output.name or attr_output.display_properties.get('display_name'),
                attr_output
            )
        else:
            attr_output = self.set_attribute_output(
